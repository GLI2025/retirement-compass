...
export function applySpendingRule(
  ...
  if (rule === 'guardrails') {
    const g = inputs.guardrails ?? { lowerBand: 0.8, upperBand: 1.2, cutPct: 0.10, raisePct: 0.10 };
    ...
    if (ctx.portfolioBalance < lower) w = w * (1 - g.cutPct);
    if (ctx.portfolioBalance > upper) w = w * (1 + g.raisePct);
  ...
  // If user picked a target age earlier than current age, fail safe to fixed behavior
  if (targetAge <= ctx.age) return Math.max(0, ctx.baselinePortfolioWithdrawal);

  // Plan to trend toward ~0 by target age:
  // Simple v1: withdraw balance / remainingMonths (straight-line).
  // Still respect baseline need as a *floor* so pension holders aren’t forced to under-spend.
